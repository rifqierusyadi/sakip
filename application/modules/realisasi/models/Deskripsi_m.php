...
class Deskripsi_m extends MY_Model
{
	public $table = 'pohon_deskripsi'; // you MUST mention the table name
	...
	function get_bidang_id($id=null)
    {
		$this->db->select('jabatan');
		$this->db->where('indikator_id', $id);
		$this->db->where('deleted_at', NULL);
        $query = $this->db->get('pohon_jabatan');
		if($query->num_rows() > 0){
			return $query->result();	
		}else{
			//show_404();
