...
class Target extends CI_Controller {
	...
	public $folder = 'realisasi/target/';
	
	public function __construct()
	{
		parent::__construct();
		$this->load->model('target_m', 'data');
		signin();
		//admin();
	}
	...
	{
		$data['head'] 		= 'Target Indikator Kinerja';
		$data['record'] 	= $this->data->get_all();
		$data['content'] 	= $this->folder.'default';
		$data['style'] 		= $this->folder.'style';
		$data['js'] 		= $this->folder.'js';
		redirect('realisasi/indikator');
		$this->load->view('template/default', $data);
	}
	
	public function created($id=null, $tahun=null)
	{
		$data['head'] 		= 'Tambah Target Indikator Kinerja';
	...
	public function updated($id=null, $tahun=null)
	{
...
// 				//add html for action
//             $col[] = '<a class="btn btn-xs btn-flat btn-info" href="'.site_url('rpjmd/target/target/'.$row->id).'" data-toggle="tooltip" title="Target"><i class="glyphicon glyphicon-stats"></i></a> <a class="btn btn-xs btn-flat btn-warning" onclick="edit_data();" href="'.site_url('rpjmd/target/updated/'.$row->id).'" data-toggle="tooltip" title="Edit"><i class="glyphicon glyphicon-pencil"></i></a>
...
//     }
	...
	public function ajax_save($id=null)
    {
			if($this->validation($id)){
				$data = array(
					...
					'tw1' => $this->input->post('tw1'),
					'tw2' => $this->input->post('tw2'),
					'tw3' => $this->input->post('tw3'),
					'tw4' => $this->input->post('tw4'),
				);
				$insert = $this->db->insert('pohon_realisasi', $data);
				//$this->db->insert_batch('pohon_detail', $result);
				helper_log("add", "Menambah Realisasi Indikator Kinerja");
			}
    }
    ...
    public function ajax_update($id=null)
    {
        if($this->validation($id)){
			...
			$idx = $this->db->get_where('pohon_realisasi',array('indikator_id'=>$data['indikator_id'],'tahun'=>$data['tahun']))->row()->id;
			$this->db->where('id', $idx);
			$this->db->update('pohon_realisasi', $data);
			helper_log("edit", "Merubah target Indikator Kinerja");
        }
    }
    ...
    // {
    //     $this->data->delete($id);
	// 	helper_log("trash", "Menghapus Target Indikator Kinerja");
	...
	private function validation($id=null)
    {
		...
		$this->form_validation->set_rules("indikator_id", "Indikator Indikator Kinerja", "trim|required");
    ...
    //     }else{
    //         echo form_dropdown('visi_id', array(''=>'Pilih Visi Indikator Kinerja'), '', "class='form-control select2' name='visi_id' id='visi_id'");
    //     }
    // }
    ...
    //     }else{
    //         echo form_dropdown('misi_id', array(''=>'Pilih Misi Indikator Kinerja'), '', "class='form-control select2' name='misi_id' id='misi_id'");
    //     }
    // }
    ...
    //     }else{
    //         echo form_dropdown('tujuan_id', array(''=>'Pilih Tujuan Misi Indikator Kinerja'), '', "class='form-control select2' name='tujuan_id' id='tujuan_id'");
    //     }
    // }
    ...
    //     }else{
    //         echo form_dropdown('sasaran_id', array(''=>'Pilih Sasaran Tujuan Indikator Kinerja'), '', "class='form-control select2' name='sasaran_id' id='sasaran_id'");
    //     }
    // }
	...
	// 					$data = array('class'=>'form-control','name'=>'target[]','id'=>'target','type'=>'text','value'=>set_value('target[]'),'placeholder'=>'target Sasaran Indikator Kinerja');
