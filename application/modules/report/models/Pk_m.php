...
class Pk_m extends MY_Model
{
	...
	public function get_data($periode=null, $tahun=null,$satker=null,$jabatan=null)
	{
		...
		$query = $this->db->query("SELECT a.id, a.deskripsi, a.sumber, b.indikator, b.id as indikator_id, c.sasaran, d.jabatan, e.target , b.satuan_id FROM sakip_pohon_deskripsi a LEFT JOIN sakip_pohon_indikator b ON a.indikator_id = b.id AND b.deleted_at is NULL LEFT JOIN sakip_pohon_jabatan d ON a.indikator_id = d.indikator_id AND d.deleted_at is NULL LEFT JOIN sakip_pohon_detail e ON a.indikator_id = e.indikator_id AND e.deleted_at is NULL LEFT JOIN sakip_pohon c ON c.id = b.sasaran_id AND c.deleted_at is NULL WHERE b.periode_id = '{$periode}' AND b.satker_id = '{$satker}' AND d.jabatan = '{$jabatan}' AND e.tahun = '{$tahun}' AND a.deleted_at is NULL GROUP BY a.indikator_id ORDER BY `a`.`id` ASC");
		if($query->num_rows() > 0)
		{
			return $query->result();
		}else{
