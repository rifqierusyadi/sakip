...
class Pohon_m extends MY_Model
{
	...
	{
		$query = $this->db->query("SELECT a.parent_id, a.kode, (CASE WHEN a.parent_id IS NULL THEN a.kode ELSE a.parent_id END) AS sort, a.order_id, a.satker, b.jabatan, b.tmt, b.eselon, b.nip  FROM simpeg_ref_satker a LEFT JOIN simpeg_jabatan_akhir b ON a.kode = b.satker_id LEFT JOIN simpeg_pangkat_akhir c ON b.nip = c.nip LEFT JOIN simpeg_identitas d ON b.nip = d.nip where a.unker_id = {$id} AND d.status_id IN(1,2) ORDER BY a.kode ASC, sort ASC, b.eselon ASC, c.gol DESC");
		if($query->num_rows() > 0)
		{
			return $query->result();
		}else{
